...
export const fechas_finales_eje = async (req, res) => {
  try {
    ...
    // Obtenemos las fechas mínima y máxima de todas las partidas de todas las etapas
    const fechasInicio = [];
    const fechasFin = [];

    obra.Etapas.forEach((etapa) => {
      etapa.Partidas.forEach((partida) => {
        fechasInicio.push(new Date(partida.Fechas.Plan.Inicio).getTime());
        fechasFin.push(new Date(partida.Fechas.Plan.Fin).getTime());
      });
    });

    const fechaInicioObra = new Date(Math.min(...fechasInicio));
    const fechaFinObra = new Date(Math.max(...fechasFin));

    // Actualizamos las fechas generales de la obra
    obra.Fechas.Ejecucion.Inicio = fechaInicioObra;
    obra.Fechas.Ejecucion.Fin = fechaFinObra;
